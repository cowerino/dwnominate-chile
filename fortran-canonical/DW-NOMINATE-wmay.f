...
C     READ IN CUMULATIVE NORMAL DISTRIBUTION FUNCTION
 ...
 903      CONTINUE    
     ...
     C     ZMIDOUT(NBILLS, NOMSTARTIN(1)), WEIGHTSOUT(NOMSTARTIN(1) + 1)
      dimension ID1(54001),LVOTE(3600),
...
C  ******************************************
  ...
  49  CONTINUE
     ...
     C           NFIRST,NLAST,NMODEL,KXTOT,XBETA,
...
C  ***************************************************************************
C
      SUBROUTINE SIGMAS(XPLOG,NFIRST,NLAST)
      use xxcom_mod
      use mine_mod
      NINC=15
      XINC=.1
...
C  ***************************************************************************
C
      SUBROUTINE WINT(XPLOG,NFIRST,NLAST)
      use xxcom_mod
      use mine_mod
      NINC=15
      XINC=.01
...
C  ***************************************************************************
C
      SUBROUTINE PLOG(XPLOG,NFIRST,NLAST)
      use xxcom_mod
      use mine_mod
      use zdf_mod
...
C
     ...
     C          XBETA(5,5),OUTX0(99,99),AADERV(99),
     C          OUTX1(99,99),OUTX2(99,99),OUTX3(99,99)
  100 FORMAT(8F7.3)
...
C
     ...
     C                 NFIRST,NLAST,NNMODEL,KXTOT,XBETA,OUTX0S,
     C                 OUTX1S,OUTX2S,OUTX3S,DERVISH)
      use xxcom_mod
      use mine_mod
      DIMENSION ATIME(152,127),
     C            YYY(152),VVV(152),ID1(54001),XMARK(99,3),
     C            XDERV(99),XDERV1(99),XDERV2(99),XDERV3(99),
     C            XXX(152),DERVISH(99,99),
     C            LLL(152),YLOG(152),YGAMMA(152,99),
     C            XXXSAVE(152,99),XBETA(5,5),XBETASV(5,5),
     C            YGMP(152),YYGMP(152),OUTX0S(99,99),
     C            OUTX1S(99,99),OUTX2S(99,99),OUTX3S(99,99)
      DOUBLE PRECISION OUTX0(99,99), OUTX1(99,99), OUTX2(99,99),
     $     OUTX3(99,99), ZMAT(99,99), WVEC(199), WORK(26*99)
...
C     Have to convert these from float/single to double so I can use R's
C     built-in LAPACK routines
      OUTX0=DBLE(OUTX0S)
      OUTX1=DBLE(OUTX1S)
      OUTX2=DBLE(OUTX2S)
      OUTX3=DBLE(OUTX3S)
...
C
     ...
     C        XDERV,XDERV1,XDERV2,XDERV3,
     C        XBETASV,REAL(OUTX0),REAL(OUTX1),REAL(OUTX2),REAL(OUTX3),
     C        NFIRST,NLAST,NMODEL)
C
...
C
  ...
  243    CONTINUE
         ...
         CALL PROX(NEP,KRC,ATIME,XMARK,XPLOG,
     C           XDERV,XDERV1,XDERV2,XDERV3,
     C           XBETA,REAL(OUTX0),REAL(OUTX1),REAL(OUTX2),REAL(OUTX3),
     C           NFIRST,NLAST,NMODEL)
C
         GMP=EXP(XPLOG/FLOAT(KRC))
C
...
C
         CALL PROX(NEP,KRC,ATIME,XMARK,XPLOG,
     C           XDERV,XDERV1,XDERV2,XDERV3,
     C           XBETASV,REAL(OUTX0),REAL(OUTX1),REAL(OUTX2),
     C           REAL(OUTX3),NFIRST,NLAST,NMODEL)
C
C  SAVE INITIAL LOG-LIKELIHOOD
C
         IF(IIII.EQ.1)THEN
...
C     code of mine and I honestly don't remember
      LWORK = 26*99
      CALL DSYEV('V', 'U', NS, OUTX0, 99, WVEC, WORK, LWORK, INFO)
      ZMAT = OUTX0
C
C  (X'X)-1
C
...
C
      IF(NEPCONG.GE.5)THEN
         CALL DSYEV('V', 'U', 2*NS, OUTX1, 99, WVEC, WORK, LWORK, INFO)
         ZMAT = OUTX1
C
C  (X'X)-1
C
...
C
      IF(NEPCONG.GE.6)THEN
         CALL DSYEV('V', 'U', 3*NS, OUTX2, 99, WVEC, WORK, LWORK, INFO)
         ZMAT = OUTX2
C
C  (X'X)-1
C
...
C
      IF(NEPCONG.GE.7)THEN
         CALL DSYEV('V', 'U', 4*NS, OUTX3, 99, WVEC, WORK, LWORK, INFO)
         ZMAT = OUTX3
C
C  (X'X)-1
C
...
C
      ...
      DIMENSION A(152,127),Y(152),C(127,127),V(152),
     C     BB(127,152)
      DOUBLE PRECISION B(127,127), WVEC(127), WORK(26*127),
     C     ZMAT(127,127)
      COMMON /AREA1/ JR,LR
...
C
  ...
  1   B(J,JJ)=SUM
      LWORK = 26*127
      CALL DSYEV('V', 'U', NF, B, 127, WVEC, WORK, LWORK, INFO)
      ZMAT = B
C
C  (X'X)-1
C
...
C
     ...
     C                             XPLOG,OLDZ,OLDD)
      use xxcom_mod
      use mine_mod
      DIMENSION DDERVX(99),ZDERVX(99),OLDZ(99),
...
C
     ...
     C          XJCH(25),XJEH(25),XJCL(25),XJEL(25),
     C          ZS(2901),UUU(1001,25),
     C          FV1(1001),FV2(1001),SUMX(1001),
     C          LWRONG(1001),LDATA(1001,2901)
      DOUBLE PRECISION Y16MIDP(1001,25), X16MIDP(1001,25), YHAT(61913),
     $     VVV(25,25), WORK(2*(4*NS*NS+7*NS))
      INTEGER IWORK(8*NS)
...
C
      LWORK = SIZE(WORK)
      CALL DGESDD('S', NP, NS, Y16MIDP, 1001, YHAT, Y16MIDP,
     $     1001, VVV, 25, WORK, LWORK, IWORK, INFO)
C
      DO 115 K=1,NS
      SUMX(K)=SUMX(K+NS)
      ZVEC(JX,K)=VVV(NS,K)
...
C  RUN REGRESSION TO ELIMINATE DIMENSION WITH LEAST VARIANCE
C
      CALL DGESDD('S', KASTRO, NS, X16MIDP, 1001, YHAT, X16MIDP, 1001,
     $     VVV, 25, WORK, LWORK, IWORK, INFO)
C
      IF(IJL.GT.25)THEN
         DO 114 K=1,NS
         ZVEC(JX,K)=VVV(NS,K)
