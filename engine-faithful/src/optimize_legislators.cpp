    ...
    const LegislatorOptimizerConfig &config)
{
    ...
    auto runOptimizationTerm = [&](int termIndex, TemporalModel model, int numSearchPoints, bool constrainSphere)
    {
        ...
        for (int iter = 0; iter < config.maxIterations; ++iter)
        {
            ...
            // Toggle #7: XINT GMP<0.5 NS==2 GRID safeguard. Port of Fortran KLUDGE at
            ...
            if (termIndex == 0 && iter == 0 && numDim == 2 && saveGMP < 0.5)
            {
                ...
                double bestX1 = coefficients(0, 0);
                double bestX2 = coefficients(0, 1);
                double bestGMP = -1.0;
                ...
                for (int ii = 0; ii < gridDim; ++ii)
                {
                    ...
                    {
                        if (xdim1 * xdim1 + xdim2 * xdim2 < 1.0)
                        {
                            coefficients(0, 0) = xdim1;
                            coefficients(0, 1) = xdim2;
                            LegislatorDerivativesResult d = computeLegislatorDerivatives(
                                ...
                                firstPeriod,
                                lastPeriod);
                            if (d.geometricMeanProb > bestGMP)
                            {
                                bestGMP = d.geometricMeanProb;
                                bestX1 = xdim1;
                                bestX2 = xdim2;
                            }
                        }
                ...
                coefficients(0, 0) = bestX1;
                coefficients(0, 1) = bestX2;
                derivs = computeLegislatorDerivatives(
                    legislatorIndex,
                    periodInfo,
                    timeTrends,
                    coefficients,
                ...
                std::cerr << " LESS THAN 0.5 leg=" << legislatorIndex
                          << " gmp_before=" << saveGMP
                          << " gmp_after=" << derivs.geometricMeanProb
                          << " x1=" << bestX1 << " x2=" << bestX2 << "\n";

                saveGMP = derivs.geometricMeanProb;
                xbetaSv = getCoefficientsVector(coefficients, termIndex, numDim);
            }
