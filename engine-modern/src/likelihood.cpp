    ...
    const std::vector<bool> &validRollCalls)
{
    ...
    for (size_t i = 0; i < numLegislators; ++i)
    {
        ...
        for (size_t j = 0; j < numRollCalls; ++j)
        {
            ...
            const double logCdfValue = normalCDF.logCdf(zs);
            if (zs < 0.0)
            {
    ...
    const std::vector<bool> &validRollCalls)
{
    ...
    // Acumuladores globales. Se mantienen seriales incluso en un build con
    // OpenMP: WINT y SIGMAS comparan valores muy cercanos y una reducción con
    // otro orden de suma puede cambiar su trayectoria. El paralelismo se
    // aplica en DWNominate a bloques independientes de votaciones y miembros.
    double totalLL = 0.0;
    int totalVotes = 0;
    ...
    for (int i = 0; i < numLegislators; ++i)
    {
        ...
        for (size_t j = 0; j < numRollCalls; ++j)
        {
            ...
            double zs =
                beta * (std::exp(utilityChoice) - std::exp(utilityOpposite));

            bool correct =
                (std::abs(utilityChoice) <= std::abs(utilityOpposite));
            if (correct)
            {
                correctClassified++;
            }
