...
namespace
{
    ...
    {
        const int requested = std::max(4 * totalErrors, 4 * numDimensions);
        return std::min(requested, numLegislators);
    }
...
#ifdef DWNOM_USE_FORTRAN_DGESDD
        // Strict optimizer-isolation mode must use the same SVD family as
        // wmay/dwnominate SEARCH.  DGESDD('S') returns VT; its last row is the
        // direction associated with the smallest singular value.
