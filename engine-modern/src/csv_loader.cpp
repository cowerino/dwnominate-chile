...
double dwnominate_input::roundLegacyStartCoordinate(double value)
{
    ...
    // nearbyint follows the default round-to-nearest floating-point mode, the
    // same tie convention used by the historical formatted serialization.
...
PeriodData CSVLoader::loadPeriodVotes(int periodNum)
{
    ...
    std::ifstream file(path);
    if (!file.is_open() && periodNum == 1)
    {
        // Static datasets historically used votes_matrix.csv. Accept it as an
        // unambiguous alias for period 1.
        path = inputDir_ + "/votes_matrix.csv";
        file.clear();
        file.open(path);
    }
    if (!file.is_open())
...
DWNominateInput CSVLoader::buildDWNominateInput(int numPeriods, const InitializationConfig *initConfig)
{
    ...
    if (initConfig && initConfig->roundStartsToThreeDecimals)
    {
        int changedCoordinates = 0;
        for (Eigen::Index r = 0; r < input.legislatorCoords.rows(); ++r)
        {
            for (Eigen::Index k = 0; k < input.legislatorCoords.cols(); ++k)
            {
                const double original = input.legislatorCoords(r, k);
                const double rounded =
                    dwnominate_input::roundLegacyStartCoordinate(original);
                input.legislatorCoords(r, k) = rounded;
                if (rounded != original)
                {
                    ++changedCoordinates;
                }
        ...
        std::cout << "  Precision de entrada legacy: " << changedCoordinates
                  << " coordenadas cuantizadas a 3 decimales\n";
    }
