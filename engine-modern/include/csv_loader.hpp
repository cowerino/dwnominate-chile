...
namespace dwnominate_input
{
/**
 * @brief Reproduce la cuantizacion decimal de los archivos de entrada historicos.
 *
 * La version standalone escribia las coordenadas iniciales con el descriptor
 * F7.3. Esta funcion emula exclusivamente esa frontera de serializacion; no se
 * utiliza durante las iteraciones del optimizador.
 */
double roundLegacyStartCoordinate(double value);
}
...
struct InitializationConfig
{
    ...
    // Per-(leg, period) seed override. When useSeedPerPeriod is true and
    // seedPerPeriodPath is set, the loader reads a 4-column CSV
    // (legislator_id, period, coord1D, coord2D) and applies the per-period
    // value to each stacked row, falling back to (0, 0) for (legId, period)
    // tuples not in the file. Used by the dynamic Chilean pipeline
    // (REQ-001) to avoid zeroing the Legendre curvature with per-leg
    // career averages. US linear default keeps the per-leg path (above).
    // The file's period field is 1-based (matches legislatura_map.csv).
    bool useSeedPerPeriod = false;
    std::string seedPerPeriodPath;

    // Compatibilidad con los archivos del DW-NOMINATE standalone: cuantiza las
...
public:
    ...
    /**
     * @brief Carga coordenadas de W-NOMINATE per-(legislator, period) desde CSV.
     *
     * Formato esperado: legislator_id,period,coord1D,coord2D
     * El campo period es 1-based (coincide con legislatura_map.csv).
     *
     * @param path Ruta al CSV
     * @return Mapa de (legislator_id, period_1based) -> (coord1D, coord2D)
     */
    std::map<std::pair<int, int>, std::pair<double, double>>
    loadWNominateCoordinatesPerPeriod(const std::string &path);
