    ...
    bool &exitEarly)
{
    ...
    std::array<SearchPointOptimized, 25> searchPointsFixed; // REQ-006: Fortran RCINT2 NINC=25 (FOR:3067), was capped at 15
    const int numPts = std::min(config.numSearchPoints, 25);
    ...
    int &validPoints)
{
    ...
    for (int kk = 0; kk < numPts; ++kk)
    {
        ...
        // REQ-004 fix: removed projectToUnitSphere(midpoint) - Fortran PROLLC2 midpoint
        // line search has NO unit-sphere constraint; OLDZ may exceed norm 1.0 during
        // optimization (Fortran RC 1 cong 1 Chile final mid=(-1.414,-0.929) norm 1.69).
        // Clamping here caps the line search's reach and prevents convergence.
